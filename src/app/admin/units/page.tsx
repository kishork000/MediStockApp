...
import { Home as HomeIcon, LayoutGrid, Package, Users2, ShoppingCart, BarChart, PlusSquare, Activity, Settings, GitBranch, LogOut, ChevronDown, Warehouse, TrendingUp, Trash2, Building } from "lucide-react";
...
export default function UnitTypeMasterPage() {
    ...
    const fetchUnitTypes = async () => {
        setDataLoading(true);
        const typesFromDb = await getUnitTypes();
        setUnitTypes(typesFromDb);
    ...
    const getIcon = (name: string) => {
        switch (name) {
            ...
            case 'Medicine Master': return <Package />;
            ...
            <div className="flex flex-col sm:gap-4 sm:py-4 sm:pl-14">
                <header className="sticky top-0 z-10 flex h-14 items-center gap-4 border-b bg-background px-4 sm:static sm:h-auto sm:border-0 sm:bg-transparent sm:px-6">
                    ...
                    <div className="flex w-full items-center justify-between">
                        <h1 className="text-xl font-semibold">Unit Type Master</h1>
                        <div className="flex items-center gap-2">
                            <Button onClick={() => setIsAddModalOpen(true)} size="sm"><PlusSquare className="mr-2 h-4 w-4" /> Add Unit Type</Button>
                            <ThemeToggle />
                        </div>
                    </div>
                </header>
                <main className="grid flex-1 items-start gap-4 p-4 sm:px-6 sm:py-0 md:gap-8">
                    <Card>
                        <CardHeader><CardTitle>Unit Types</CardTitle><CardDescription>Manage the base measurement units for medicines (e.g., PCS, BTL, ML).</CardDescription></CardHeader>
                        <CardContent>
                            <Table>
                                ...
                                <TableBody>
                                    {dataLoading ? (Array.from({ length: 3 }).map((_, i) => (<TableRow key={i}><TableCell><Skeleton className="h-4 w-32" /></TableCell><TableCell><Skeleton className="h-8 w-8" /></TableCell></TableRow>)))
                                        : unitTypes.map((type) => (<TableRow key={type.id}><TableCell className="font-medium">{type.name}</TableCell><TableCell className="text-right"><Button variant="ghost" size="icon" onClick={() => handleDelete(type.id)}><Trash2 className="h-4 w-4 text-destructive" /></Button></TableCell></TableRow>))}
                                </TableBody>
                            </Table>
                        </CardContent>
                    </Card>
                    ...
                    <form onSubmit={handleAddUnitType}>
                        ...
                        <div className="grid gap-4 py-4">
                            <div className="space-y-2">
                                ...
                                <Input id="name" name="name" value={newUnitTypeName} onChange={(e) => setNewUnitTypeName(e.target.value)} placeholder="e.g., PCS" required />
                            </div>
                        </div>
                        <DialogFooter>
